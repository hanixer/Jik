...
Неправильно обрабатываются константы. Только первый элемент списка правильно отображается.
Остальные где-то пропадают.
Почему это происходит?
Потому что происходит GlobalRef, сохраняется значение на стек, потом происходит
GC, и в той переменной на стеке лежит невалидный адрес после сборки.
Как решить?
1) Заменить GlobalRef на прямой доступ.
2) Добавлять значения на root stack.

Как заменить на прямой доступ?
Добавить операнд.
Или делать пробежку и проверять не глобальная ли это переменная.
Т.е. бежим по сгенеренному Х86, и запоминаем какие переменные являются глобальными.
Затем все эти ссылки заменяем на прямые обращения, а сами переменные выбрасываем.
