...
; Need to handle inner definitions.
  ...
  (define (make-lattice g print?)
    ...
    (define (loop g count)
      ...
      (let ((lattice (step g)))
        (when print? (begin (display " -> ") (display (length (graph-nodes lattice)))))
        (let* ((new-g (reduce lattice))
          ...
          (if (= new-count count)
              (begin
                ...
                new-g)
              (begin
                (when print? (begin (display " -> ") (display new-count) (newline)))
  ...
  (define (test)
    (setup)
    (map name
         (graph-nodes (make-lattice (make-graph a b c d any-node none-node) #t))))
